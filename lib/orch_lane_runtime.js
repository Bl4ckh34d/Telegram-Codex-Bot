...
function createOrchLaneRuntime(deps = {}) {
  ...
  function makeWorldMonitorCheckTextSummary(text) {
    ...
    const sectionBullets = (sectionName, limit = 1) => {
      const out = [];
      let inSection = false;
    ...
    const headline = sectionBullets("HEADLINE", 1)[0] || "";
    const quintessence = sectionBullets("QUINTESSENCE", 3);
    const watch = sectionBullets("WATCH_24H", 2);
    const bottomLine = sectionBullets("BOTTOM_LINE", 1)[0] || "";

    if (!headline && quintessence.length <= 0 && watch.length <= 0 && !bottomLine) {
      return src;
    }

    const out = [];
    if (headline) out.push(`Headline: ${headline}`);
    for (const item of quintessence) out.push(`- ${item}`);
    if (watch.length > 0) {
      out.push("Watch:");
      for (const item of watch) out.push(`- ${item}`);
    }
    if (bottomLine) out.push(`Bottom line: ${bottomLine}`);
    return out.join("\n").trim();
  }
  ...
  function normalizeWorkerLabelKey(value) {
    return String(value || "").replace(/\s+/g, " ").trim().toLowerCase();
  }

  function resolveWorkerIdFromLabel(label, fallbackWorkerId = "") {
    const key = normalizeWorkerLabelKey(label);
    if (!key) return "";
    const workers = typeof listCodexWorkers === "function" ? listCodexWorkers() : [];
    for (const worker of Array.isArray(workers) ? workers : []) {
      const wid = String(worker?.id || "").trim();
      if (!wid) continue;
      const idKey = normalizeWorkerLabelKey(wid);
      const nameKey = normalizeWorkerLabelKey(worker?.name || "");
      const titleKey = normalizeWorkerLabelKey(worker?.title || "");
      if (key === idKey || key === nameKey || key === titleKey) return wid;
    }
    if (normalizeWorkerLabelKey(fallbackWorkerId) === key) return String(fallbackWorkerId || "").trim();
    return "";
  }

  function splitSpokenTextByWorkerLabels(spokenText, fallbackWorkerId = "") {
    const src = String(spokenText || "");
    if (!src.trim()) return [];
    ...
    const pushCurrent = () => {
      ...
      out.push({
        workerId: String(current.workerId || "").trim() || String(fallbackWorkerId || "").trim(),
        text,
      });
      current = null;
    };
    ...
    for (const rawLine of lines) {
      ...
      let m = cleaned.match(/^\[([^\]]{1,80})\]\s*[:\-]?\s+(.+)$/);
      if (m) {
        label = String(m[1] || "").trim();
        content = String(m[2] || "").trim();
      } else {
        m = cleaned.match(/^([A-Za-z0-9][A-Za-z0-9 _-]{0,80})\s*[:\-]\s+(.+)$/);
        if (m) {
          label = String(m[1] || "").trim();
          content = String(m[2] || "").trim();
        }
      ...
      if (label && content) {
        const workerId = resolveWorkerIdFromLabel(label, fallbackWorkerId);
        if (workerId) {
          pushCurrent();
          current = { workerId, text: content };
          taggedCount += 1;
          continue;
        }
      }
      ...
      if (!current) {
        current = {
          workerId: String(fallbackWorkerId || "").trim(),
          text: cleaned,
        };
      } else {
        current.text = `${String(current.text || "").trim()} ${cleaned}`.trim();
      }
    }
    pushCurrent();

    if (taggedCount < 2) return [];
    return out.filter((item) => String(item?.text || "").trim());
  }
  ...
  async function enqueuePrompt(chatId, inputText, source, options = {}) {
    ...
    const job = {
      ...
      splitGroupId: String(options?.splitGroupId || "").trim(),
      originMessageId: Number(options?.originMessageId || 0),
      replyToMessageId: Number(options?.replyToMessageId || 0),
      startedAt: 0,
      cancelRequested: false,
      timedOut: false,
  ...
  async function enqueueTts(chatId, inputText, source, options = {}) {
    ...
    const ttsPreset = resolveTtsPresetForChat(
      chatId,
      options?.ttsPreset || "",
      workerId,
      { forceWorkerMode: options?.forceWorkerMode === true },
    );
    const job = {
      id: nextJobId(),
      chatId,
  ...
  function startJobProgressUpdates(job, lane) {
    ...
    if (
      ...
      (Array.isArray(job?.imagePaths) && job.imagePaths.length > 0)
    ) {
      return () => {};
    }
  ...
  function canBatchQueuedPromptJob(job, firstJob) {
    if (!job || !firstJob) return false;
    if (String(job.kind || "") !== "codex" || String(firstJob.kind || "") !== "codex") return false;
    if (String(job.chatId || "") !== String(firstJob.chatId || "")) return false;
    if (String(job.workerId || "") !== String(firstJob.workerId || "")) return false;
    if (String(job.replyStyle || "") !== String(firstJob.replyStyle || "")) return false;
    if (String(job.resumeSessionId || "") !== String(firstJob.resumeSessionId || "")) return false;
    if (String(job.model || "") !== String(firstJob.model || "")) return false;
    if (String(job.reasoning || "") !== String(firstJob.reasoning || "")) return false;
    if (String(job.forcedTextOnly || "") || String(firstJob.forcedTextOnly || "")) return false;
    if (String(job.splitGroupId || "") || String(firstJob.splitGroupId || "")) return false;
    if (typeof job.onSuccess === "function" || typeof firstJob.onSuccess === "function") return false;
    return true;
  }

  function mergeQueuedPromptJobs(firstJob, lane) {
    if (!firstJob || !lane || !Array.isArray(lane.queue)) return firstJob;
    if (String(firstJob.kind || "") !== "codex") return firstJob;

    const batch = [firstJob];
    let takeCount = 0;
    for (const queued of lane.queue) {
      if (canBatchQueuedPromptJob(queued, firstJob)) {
        batch.push(queued);
        takeCount += 1;
      } else {
        break;
    ...
    if (batch.length <= 1) return firstJob;
    lane.queue.splice(0, takeCount);

    const latest = batch[batch.length - 1];
    const formatter = typeof formatQueuedPromptBatchText === "function"
      ? formatQueuedPromptBatchText
      : (jobs) => jobs.map((job, idx) => `Message ${idx + 1}:\n${String(job?.text || "").trim()}`).join("\n\n");
    const text = formatter(batch);
    const imagePaths = [];
    const seenImages = new Set();
    for (const item of batch) {
      const paths = Array.isArray(item?.imagePaths) ? item.imagePaths : [];
      for (const imgPath of paths) {
        const clean = String(imgPath || "").trim();
        if (!clean || seenImages.has(clean)) continue;
        seenImages.add(clean);
        imagePaths.push(clean);
      }
    }
    return {
      ...firstJob,
      id: firstJob.id,
      text,
      source: imagePaths.length > 0 ? "telegram-vision-batch" : "telegram-batch",
      imagePaths,
      replyContext: null,
      replyThreadContext: null,
      recentHistoryContext: null,
      replyToMessageId: Number(latest?.replyToMessageId || firstJob.replyToMessageId || 0),
      taskId: Number(firstJob?.taskId || 0),
      batchJobs: batch,
      batchSize: batch.length,
    };
  }

  function getTaskJobsForLifecycle(job) {
    const jobs = Array.isArray(job?.batchJobs) && job.batchJobs.length > 0 ? job.batchJobs : [job];
    const out = [];
    const seen = new Set();
    for (const item of jobs) {
      const taskId = Number(item?.taskId || 0);
      const key = `${String(item?.chatId || "")}:${Number.isFinite(taskId) ? Math.trunc(taskId) : 0}`;
  ...
  async function processLane(laneId) {
    ...
    while (Array.isArray(lane.queue) && lane.queue.length > 0 && !getShuttingDown()) {
      const job = mergeQueuedPromptJobs(lane.queue.shift(), lane);
      lane.currentJob = job;
      job.startedAt = Date.now();
      for (const taskJob of getTaskJobsForLifecycle(job)) {
        taskJob.startedAt = job.startedAt;
        markOrchTaskRunning(taskJob);
      }
      ...
      try {
        ...
        if (shouldVoiceReply) {
          try {
            ...
            const spoken = (isWorldMonitorAlertJob || isWorldMonitorCheckJob)
              ...
              : spokenRaw;
            const perWorkerSegments = splitSpokenTextByWorkerLabels(spoken, routeWorkerId);
            const flattenSegmentChunks = [];
            let overflowText = "";
            if (perWorkerSegments.length > 0) {
              for (const segment of perWorkerSegments) {
                const split = splitSpeakableTextIntoVoiceChunks(segment.text);
                const segmentChunks = Array.isArray(split?.chunks) ? split.chunks : [];
                for (const chunk of segmentChunks) {
                  const text = String(chunk || "").trim();
                  if (!text) continue;
                  flattenSegmentChunks.push({
                    workerId: String(segment.workerId || "").trim() || routeWorkerId,
                    text,
                  });
                }
                const extra = String(split?.overflowText || "").trim();
                if (extra) {
                  overflowText = overflowText ? `${overflowText} ${extra}` : extra;
                }
              }
            } else {
              const split = splitSpeakableTextIntoVoiceChunks(spoken);
              const spokenChunks = Array.isArray(split?.chunks) ? split.chunks : [];
              for (const chunk of spokenChunks) {
                const text = String(chunk || "").trim();
                if (!text) continue;
                flattenSegmentChunks.push({ workerId: routeWorkerId, text });
              }
              overflowText = String(split?.overflowText || "").trim();
            }
            ...
            const chunks = flattenSegmentChunks.length > 0
              ? flattenSegmentChunks
              : (spoken ? [{ workerId: routeWorkerId, text: spoken }] : []);
            const groupedChunks = [];
            for (const chunk of chunks) {
              const workerIdForChunk = String(chunk?.workerId || "").trim() || routeWorkerId;
              const textForChunk = String(chunk?.text || "").trim();
              if (!textForChunk) continue;
              const prev = groupedChunks.length > 0 ? groupedChunks[groupedChunks.length - 1] : null;
              if (prev && prev.workerId === workerIdForChunk) {
                prev.texts.push(textForChunk);
              } else {
                groupedChunks.push({ workerId: workerIdForChunk, texts: [textForChunk] });
              }
            }
            let queuedCount = 0;
            for (const group of groupedChunks) {
              const workerIdForChunk = String(group.workerId || "").trim() || routeWorkerId;
              const sessionIdForChunk = String(getSessionForChatWorker(job.chatId, workerIdForChunk) || resolvedSessionId || "").trim();
              let queued = false;
              if (group.texts.length > 1) {
                queued = await enqueueTtsBatch(job.chatId, group.texts, "voice-reply", {
                  ...
                  skipResultText: true,
                  attachments: [],
                  workerId: workerIdForChunk,
                  forceWorkerMode: perWorkerSegments.length > 0,
                  replyToMessageId,
                  routeTaskId,
                  routeSessionId: sessionIdForChunk,
                });
              } else {
                queued = await enqueueTts(job.chatId, group.texts[0], "voice-reply", {
                  // Send text/attachments from the main job path so they do not wait for TTS.
                  afterText: "",
                  // Even if TTS_SEND_TEXT=1 globally, don't duplicate the spoken content for voice replies.
                  skipResultText: true,
        ...
        if (!result || (result.skipSendMessage !== true && !shouldSuppressResultTextForQueuedVoice)) {
          try {
            const responseBaseText = shouldSendForcedTextOnly
              ? forcedTextOnly
              : normalizeResponse(result?.text);
            const responseText = isWorldMonitorCheckJob
              ? makeWorldMonitorCheckTextSummary(responseBaseText)
              : responseBaseText;
        ...
        for (const taskJob of getTaskJobsForLifecycle(job)) {
          markOrchTaskCompleted(taskJob, result, taskContext);
        }
