...
function createOrchRouterRuntime(deps = {}) {
  ...
  function buildDelegationAckText(assignments, stateSummary = null) {
    ...
    if (rows.length === 1) {
      return summary.allQueued ? "In queue." : "In progress.";
    }
    const laneStateByWorker = new Map();
    const lines = [];
    for (const row of rows) {
      const wid = String(row.workerId || "").trim();
      if (!wid) continue;
      const label = describeWorkerForDelegation(wid) || wid;
      ...
      const willQueue = state.busy || state.queued > 0;
      if (willQueue) state.queued += 1;
      else state.busy = true;
      laneStateByWorker.set(wid, state);

      lines.push(`${label}: ${willQueue ? "In queue." : "In progress."}`);
    }
    return lines.join("\n");
  }
  ...
  function buildRouterPrompt({
    ...
    lines.push("", "Worker capabilities:");
    for (const w of Array.isArray(workers) ? workers : []) {
      const wid = String(w?.id || "").trim();
      if (!wid) continue;
      const summary = typeof getWorkerCapabilitySummary === "function"
        ? String(getWorkerCapabilitySummary(wid) || "").trim()
        : "";
      if (!summary) continue;
      lines.push(`- ${wid}: ${summary}`);
    }
  ...
  function parseRouterOutput(text) {
    ...
    const markerMatch = /ROUTE\s*:/i.exec(src);
    if (!markerMatch) return null;
    const markerIndex = Number(markerMatch.index || 0);
    const after = src.slice(markerIndex + markerMatch[0].length);
  ...
  function looksLikeMultiTaskRequest(text) {
    const src = String(text || "");
    if (!src.trim()) return false;

    const listItems = src
      .split(/\r?\n/)
      .filter((line) => /^\s*(?:\d+[\).:]|[-*])\s+\S+/.test(String(line || "")));
    if (listItems.length >= 2) return true;

    const lower = src.toLowerCase();
    const intentHints = [
      /\b(first\b.*\bsecond)\b/s,
      /\b(two|three|multiple|several)\s+(tasks?|steps?|requests?|commands?)\b/,
      /\b(and also|also|plus|in addition|separately|at the same time|in parallel)\b/,
    ];
    if (intentHints.some((re) => re.test(lower))) return true;

    const clauseBreaks = src.split(/[;]+/).map((x) => String(x || "").trim()).filter(Boolean);
    return clauseBreaks.length >= 2;
  }
  ...
  async function routeAndEnqueuePrompt(chatId, userText, source, options = {}) {
    ...
    const allowSplit = src === "voice" || src === "whisper"
      ? looksLikeMultiTaskRequest(userText)
      : true;
