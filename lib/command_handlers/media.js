...
function createMediaCommandHandlers(deps = {}) {
  ...
  return {
    ...
    "/ask": async (chatId, parsed) => {
      ...
      const recentImages = typeof getRecentImagesForChat === "function"
        ? getRecentImagesForChat(chatId)
        : [];
      const fallbackImage = getLastImageForChat(chatId);
      const imagePaths = recentImages.length > 0
        ? recentImages.map((img) => String(img?.path || "").trim()).filter(Boolean)
        : [String(fallbackImage?.path || "").trim()].filter(Boolean);
    ...
    "/see": async (chatId, parsed) => {
      ...
      void (async () => {
        try {
          const captureDir = String(OUT_DIR || "").trim() || IMAGE_DIR;
          const recentImagePaths = typeof getRecentImagesForChat === "function"
            ? getRecentImagesForChat(chatId).map((img) => String(img?.path || "").trim()).filter(Boolean)
            : [];
          const paths = await captureScreenshotsWithFallback(captureDir, prefix);
          ...
          const imagePaths = [...new Set([...recentImagePaths, ...paths.map((p) => String(p || "").trim()).filter(Boolean)])];
          const activeSession = getActiveSessionForChat(chatId);
